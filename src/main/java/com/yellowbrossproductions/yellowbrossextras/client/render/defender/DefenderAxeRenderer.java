...
public class DefenderAxeRenderer<T extends DefenderAxe> extends EntityRenderer<T> {
    ...
    public void render(T entity, float yaw, float partialTick, PoseStack poseStack, MultiBufferSource bufferSource, int blockLight) {
        ...
        poseStack.mulPose(Axis.ZP.rotationDegrees(-(entity.tickCount + partialTick) * 25));

        this.itemRenderer.renderStatic(null, RENDER_ITEM, ItemDisplayContext.FIXED, false, poseStack, bufferSource, entity.level(), blockLight, OverlayTexture.NO_OVERLAY, entity.getId());
