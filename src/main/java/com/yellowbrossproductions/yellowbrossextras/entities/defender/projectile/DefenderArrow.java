...
public class DefenderArrow extends AbstractArrow {
    private static final EntityDataAccessor<Integer> ARROW_TYPE = SynchedEntityData.defineId(DefenderArrow.class, EntityDataSerializers.INT);

    public DefenderArrow(EntityType<? extends AbstractArrow> arrow, Level level) {
        super(arrow, level);
    }

    public DefenderArrow(Level level, double x, double y, double z) {
        super(YEEntityTypes.DefenderArrow.get(), x, y, z, level);
        this.setPos(x, y, z);
    }

    public DefenderArrow(Level level, LivingEntity entity) {
        ...
        for (Entity entity : list) {
            if (entity instanceof LivingEntity living) {
                ...
                if (team && entity.isAlive() && !entity.isInvulnerable() && !entity.isSpectator()) {
                    living.hurt(this.damageSources().arrow(this, this.getOwner()), (float) this.getBaseDamage() * EntityUtil.multiplyToScrewArmor(living, 1.0f));
                }
            }
        }
    }
