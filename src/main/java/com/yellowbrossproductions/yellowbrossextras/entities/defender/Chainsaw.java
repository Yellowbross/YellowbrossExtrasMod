...
public class Chainsaw extends Entity {
    ...
    private static final EntityDataAccessor<Float> YAW = SynchedEntityData.defineId(Chainsaw.class, EntityDataSerializers.FLOAT);

    private static final EntityDataAccessor<Float> PITCH = SynchedEntityData.defineId(Chainsaw.class, EntityDataSerializers.FLOAT);

    private static final EntityDataAccessor<Integer> DURATION = SynchedEntityData.defineId(Chainsaw.class, EntityDataSerializers.INT);

    private static final EntityDataAccessor<Integer> CASTER = SynchedEntityData.defineId(Chainsaw.class, EntityDataSerializers.INT);
    ...
    public Chainsaw(EntityType<? extends Chainsaw> entityType, Level level) {
    ...
    public Chainsaw(EntityType<? extends Chainsaw> type, Level world, LivingEntity caster, double x, double y, double z, float yaw, float pitch, int duration) {
        this(type, world);
        this.caster = caster;
        this.setYaw(yaw);
        this.setPitch(pitch);
    ...
    private void updateWithDefender() {
        if (this.caster != null) {
            ...
            if (caster instanceof Defender) {
                this.setPitch((float) ((-((Defender) caster).getChainsawLookX()) * Math.PI / 180.0d));
            }
    ...
    public void tick() {
        ...
        if (caster instanceof Defender) {
            ...
            renderPitch = (float) ((-((Defender) caster).getChainsawLookX()) * Math.PI / 180.0d);
        }
